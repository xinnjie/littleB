...
public:
    ...
    bool RegisterSyncService(CmdId cmd_id, std::unique_ptr<SyncServiceInterface> &&service) {
        auto iter = sync_services_.emplace(std::make_pair(cmd_id, std::move(service)));  // TODO std::forward为什么不行
        return iter.second;
    }

    bool RegisterAsyncService(CmdId cmd_id, std::unique_ptr<AsyncServiceInterface> &&service) {
        auto iter = async_services_.emplace(std::make_pair(cmd_id, std::move(service)));  // TODO std::forward为什么不行
        return iter.second;
    }
    // TODO 可能需要工厂，因为有状态的
    // service 之间的状态是隔离的
