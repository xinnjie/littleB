...
LengthFieldBasedFrameDecoder::LengthFieldBasedFrameDecoder(uint32_t lengthFieldLength, uint32_t maxFrameLength,
                                                           uint32_t lengthFieldOffset, int32_t lengthAdjustment,
                                                           uint32_t initialBytesToStrip, bool networkByteOrder)
...
bool LengthFieldBasedFrameDecoder::decode(Context* ctx, IOBufQueue& buf, std::unique_ptr<IOBuf>& result, size_t&) {
    // discarding too long frame
    if (buf.chainLength() < lengthFieldEndOffset_) {
        return false;
    }

    uint64_t frameLength = getUnadjustedFrameLength(buf, lengthFieldOffset_, lengthFieldLength_, networkByteOrder_);
    ...
    if (frameLength < lengthFieldEndOffset_) {
        ...
        ctx->fireReadException(folly::make_exception_wrapper<std::runtime_error>("Frame too small"));
        return false;
    }
    ...
    if (initialBytesToStrip_ > frameLength) {
        ...
        ctx->fireReadException(
            folly::make_exception_wrapper<std::runtime_error>("InitialBytesToSkip larger than frame"));
        return false;
    }
    ...
    SPDLOG_INFO("pkg size={}|length in head={}", result->length(), frameLength);
    return true;
}

uint64_t LengthFieldBasedFrameDecoder::getUnadjustedFrameLength(IOBufQueue& buf, int offset, int length,
